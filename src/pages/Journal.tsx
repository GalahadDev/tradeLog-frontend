  ...
  const filteredTrades = useMemo(() => {
    return trades.filter((trade) => {
      ...
      if (dateFrom || dateTo) {
        ...
        const start = dateFrom ? startOfDay(parseISO(dateFrom)) : new Date(2000, 0, 1);
        const end = dateTo ? endOfDay(parseISO(dateTo)) : new Date(2100, 0, 1);
          ...
          <DialogContent className="max-w-4xl p-0 overflow-hidden bg-transparent border-none shadow-none flex justify-center items-center">
            {viewImage && (
              <div className="relative">
                <button onClick={() => setViewImage(null)} className="absolute -top-4 -right-4 bg-black/50 hover:bg-black/80 text-white rounded-full p-2 backdrop-blur-md transition-colors z-50">
                  <X className="w-5 h-5" />
                </button>
                <img src={viewImage} alt="Trade Screenshot" className="max-h-[85vh] w-auto rounded-lg shadow-2xl border border-white/10"/>
              </div>
            )}
          </DialogContent>
        </Dialog>
            ...
            <div className="flex gap-2 col-span-1 lg:col-span-2">
               <div className="relative flex-1">
                  ...
                  <Input 
                    ...
                    className="pl-8 bg-black/20 border-white/10 text-xs"
                  />
               </div>
               <div className="relative flex-1">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground/50 text-[10px] uppercase">A</span>
                  <Input 
                    type="date" 
                    value={dateTo}
                      ...
                      <TableCell className="text-center">
                        {trade?.screenshot_url ? (
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            onClick={() => setViewImage(trade.screenshot_url!)}
